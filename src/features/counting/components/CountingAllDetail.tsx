...
import {CountingAllDetailProps, useCountingAllDetailsData} from "@/features/counting/hooks/useCountingAllDetailsData";
import {useDateTimeFormat} from "@/hooks/useDateTimeFormat";
import {Label, useAuth} from "@/components";
import {formatNumber} from "@/utils/number-utils";
import {getQuantityStep} from "@/utils/quantity-utils";

const CountingAllDetail = forwardRef((props: CountingAllDetailProps, ref) => {
  ...
  const {
    ...
    setIsOpen
  } = useCountingAllDetailsData(props);
  const enableDecimals = user?.settings?.enableDecimalQuantities ?? false;

  useImperativeHandle(ref, () => ({
    show(data: CountingSummaryReportLine, details: any[], enableUpdate: boolean) {
      setCurrentData(data);
      ...
      <DialogContent className={displayBarcode ? "sm:max-w-4xl" : "sm:max-w-3xl"}>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {currentData?.binCode && ` | ${t('bin')}: ${currentData.binCode}`}
          </DialogDescription>
        </DialogHeader>
...
export default CountingAllDetail;
