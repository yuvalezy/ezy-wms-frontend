        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t('buyUnit')}</TableHead>
              <TableHead className="text-right">{t('packUnit')}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {data.map((binContent, index) => {
              ...
              return (
                <TableRow key={index}>
                  ...
                  <TableCell>{binContent.itemName}</TableCell>
                  <TableCell className="text-right">{formatNumber(binContent.onHand)}</TableCell>
                  <TableCell className="text-right">
                    {formatNumber(binContent.onHand / numInBuy, 2)} {wd(binContent.buyUnitMsr, t('buyUnit'))}
                  </TableCell>
                  <TableCell className="text-right">
                    {formatNumber(binContent.onHand / (numInBuy * purPackUn), 2)} {wd(binContent.purPackMsr, t('packUnit'))}
                  </TableCell>
                </TableRow>
              );
            })}
