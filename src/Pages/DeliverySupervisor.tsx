...
import {DialogDomRef, MessageStripDesign, Panel, PanelDomRef, Table, TableColumn, Label, TableRow, TableCell} from "@ui5/webcomponents-react";
import {Document, DocumentAction} from "../Assets/Document";
import {DeliveryOpenDocument} from "../Assets/Deliveries";
import {DeliveryOpenDocuments} from "../Assets/mockup";
...
export default function DeliverySupervisor() {
    ...
    const [data, setData] = useState<DeliveryOpenDocument[]>(
        Array.from({ length: Math.ceil(100 / DeliveryOpenDocuments.length) }, () => DeliveryOpenDocuments).flatMap(item => item)
    );
    const objectName = useObjectName();

    return (
        <ContentTheme title={t("deliverySupervisor")} icon="kpi-managing-my-area">
            <Panel
                collapsed={isPanelCollapsed}
                onToggle={() => setIsPanelCollapsed(!isPanelCollapsed)}
                ref={panelRef}
                headerText={t("openDocuments")}
            >
                <Table
                    columns={<>
                        <TableColumn><Label>{t('type')}</Label></TableColumn>
                        <TableColumn><Label>{t('number')}</Label></TableColumn>
                        <TableColumn><Label>{t('date')}</Label></TableColumn>
                        <TableColumn><Label>{t('customer')}</Label></TableColumn>
                        <TableColumn><Label>{t('customerName')}</Label></TableColumn>
                    </>}
                >
                    {data.map((row) => (
                        <TableRow key={row.docEntry}>
                            <TableCell><Label>{objectName(row.objType)}</Label></TableCell>
                            <TableCell><Label>{row.docNum}</Label></TableCell>
                            <TableCell><Label>{row.docDate.toLocaleDateString()}</Label></TableCell>
                            <TableCell><Label>{row.cardCode}</Label></TableCell>
                            <TableCell><Label>{row.cardName}</Label></TableCell>
                        </TableRow>
                    ))}
                </Table>
            </Panel>
